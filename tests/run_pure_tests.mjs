...
/* ── MAGIC REBUILD (ruled 2026-08-23, design-magic-economy-resim) ──────── */
...
// binds at 63% (triage), chips rationally eaten, 3v1 in 1.2r.
{
  ...
  // full +185 costs 50 stamina; half pays half; the cap holds at double.
  eq('bulwark: full brace doubles the wall', F3.bulwarkWallBonus(185, 50, 1000, 0.05, 1.0), 185);
  eq('bulwark: half brace, half bonus', F3.bulwarkWallBonus(185, 25, 1000, 0.05, 1.0), 93);
  eq('bulwark: overpay caps at maxMult', F3.bulwarkWallBonus(185, 200, 1000, 0.05, 1.0), 185);
  ...
  // Dive invest (ruled 2026-08-23) rides the SAME bulwarkWallBonus math on
  // the dodge value — one grammar for wall and legs.
  eq('dive: shipped boost cap 1.0', Number(/diveMaxBoostMult:\s*([\d.]+)/.exec(src)?.[1]), 1.0);
  eq('dive: half-price invest buys half the dodge',
     F3.bulwarkWallBonus(600, 27, 1070, 0.05, 1.0), Math.round(600 * (27 / (0.05 * 1070))));
