...
class CharacterDataManager:
    ...
    def calculate_race_bonuses_only(self, race_level: int) -> Dict[str, Any]:
        """
        Calculate ONLY race bonuses using race history (centralized race logic).
        
        Args:
            race_level: Maximum race level to calculate bonuses for
            
        Returns:
            Dictionary containing:
            - stats: Dict with stat bonuses for each stat
            - free_points: Total free points from race progression
        """
        from game_data import races
        ...
        # Calculate bonuses for each level using race history
        for level in range(1, race_level + 1):
            race_at_level = self.get_race_at_race_level(level)
                ...
                for range_data in sorted(rank_ranges, key=lambda x: x["min_level"]):
                    if range_data["min_level"] <= level <= range_data["max_level"]:
                        for stat, gain in range_data.get("stats", {}).items():
                            if stat == "free_points":
                                result["free_points"] += gain
                            elif stat in STATS:
                                result["stats"][stat] += gain
                        break
        
        return result
    
    def calculate_race_free_points_only(self, race_level: int) -> int:
        """
        Calculate ONLY free points from race progression (optimized for when stats aren't needed).
        
        Args:
            race_level: Maximum race level to calculate for
            
        Returns:
            Total free points from race progression
        """
        from game_data import races
        ...
        # Calculate free points for each level using race history
        for level in range(1, race_level + 1):
            race_at_level = self.get_race_at_race_level(level)
            if race_at_level and race_at_level.lower() in races:
                race_data = races[race_at_level.lower()]
                ...
                for range_data in sorted(rank_ranges, key=lambda x: x["min_level"]):
                    if range_data["min_level"] <= level <= range_data["max_level"]:
                        free_points_gain = range_data.get("stats", {}).get("free_points", 0)
                        total_free_points += free_points_gain
                        break
        
        return total_free_points
    
    def get_race_rank_for_level(self, race_level: int) -> str:
        """
        Get the race rank for a specific race level.
        
        Args:
            race_level: The race level to get rank for
            
        Returns:
            Race rank string, or empty string if no rank found
        """
        from game_data import races
        ...
        race_at_level = self.get_race_at_race_level(race_level)
        if not race_at_level or race_at_level.lower() not in races:
            return ""
        
        race_data = races[race_at_level.lower()]
        rank_ranges = race_data.get("rank_ranges", [])
        
        # Find the applicable rank range for this level
        for range_data in sorted(rank_ranges, key=lambda x: x["min_level"]):
            if range_data["min_level"] <= race_level <= range_data["max_level"]:
                return range_data.get("rank", "")
...
class LevelSystem:
    ...
    def _apply_race_level_up(self, from_level: int, to_level: int, skip_free_points: bool = False) -> None:
        ...
        if not self.data_manager.race_history:
            # No race history, use current race for all levels - inline the loop
            current_race = self.data_manager.get_meta("Race", "").lower()
            if current_race:
                for level in range(from_level + 1, to_level + 1):
                    self._apply_race_bonuses_for_level(current_race, level, skip_free_points)
            return
...
class StatValidator:
    ...
    def _calculate_expected_race_free_points(self) -> int:
        """Calculate expected free points from race bonuses only (for familiars/monsters) - CENTRALIZED"""
        race_level = int(self.character.data_manager.get_meta("Race level", "0"))
        
        # Use centralized race calculation
        total_race_free_points = self.character.data_manager.calculate_race_free_points_only(race_level)
        
        # Calculate how many free points should remain after spending
        ...
        return total_race_free_points - free_points_spent
    ...
    def _validate_race_leveling_character(self) -> Dict[str, Any]:
        ...
        # 3. ENHANCED: Calculate detailed expected stats from race bonuses (like regular characters) - CENTRALIZED
        race_level = int(self.character.data_manager.get_meta("Race level", "0"))
        race_bonuses = self.character.data_manager.calculate_race_bonuses_only(race_level)
        
        expected_race_stats = race_bonuses["stats"]
        expected_race_stats["free_points"] = race_bonuses["free_points"]  # For backward compatibility
        ...
        expected_free_points = race_bonuses["free_points"]
    ...
    def calculate_expected_bonuses(self) -> Dict[str, Any]:
        ...
        # For familiars/monsters, only calculate race bonuses - CENTRALIZED
        if self.character.data_manager.is_race_leveling_type():
            race_level = int(self.character.data_manager.get_meta("Race level", "0"))
            race_bonuses = self.character.data_manager.calculate_race_bonuses_only(race_level)
            
            # Apply race bonuses to result structure
            bonuses["race"] = race_bonuses["stats"]
            bonuses["race_free_points"] = race_bonuses["free_points"]
        ...
        # Calculate race bonuses using race history - CENTRALIZED
        race_level = int(self.character.data_manager.get_meta("Race level", "0"))
        race_bonuses = self.character.data_manager.calculate_race_bonuses_only(race_level)
