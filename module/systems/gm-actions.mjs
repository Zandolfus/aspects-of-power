...
export async function executeGmAction(payload) {
    ...
    switch (payload.type) {
      ...
      case 'gmApplyDebuff': {
        ...
        // ── Debuff BUILD-UP: a stacking debuff becomes a worse one ──
        // Was a hand-wired Chilled → Frozen block here; now driven by
        // CONFIG.debuffBuildup so a second pair is a config entry rather
        // than another branch in this switch. Behaviour is unchanged for
        // chilled: sum the live stacks, and at the dexterity mod replace
        // them all with Frozen.
        await applyDebuffBuildup(target, payload.effectData?.system?.debuffType, {
          startRound, startTurn,
