...
export class AspectsofPowerActorSheet extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  /**
   * Declare a manual break-free attempt on the celerity stack. Replaces the
   * legacy "1-of-3 actions per turn" gate with a celerity wait derived from
   * the actor's relevant break-stat mod. Roll fires when the tracker reaches
   * the scheduled tick (see celerity-combat-tracker BREAK_FREE_ITEM_ID branch).
  ...
  async _onBreakFree(effect) {
    ...
    await declareBreakFree(actor, effect);
  }
