  ...
  async _onChangeForm(formConfig, event) {
    ...
    if (this.item.type === 'skill' && (event.target?.name?.startsWith('system.tagConfig.') || event.target?.classList?.contains('attr-value'))) {
      ...
      const tagConfigData = {
        ...
        // STACKS — enumerated rather than left to the generic catch-all,
        // which does `Number(el.value) || 0`: a blank Scaling box would coerce
        // 1.0 (linear) to 0 (spent**0 = flat 1x), silently unscaling the skill.
        stackPool:         str('stackPool', ''),
        stackProduces:     numOrZero('stackProduces', 0),
        stackCost:         numOrZero('stackCost', 0),
        stackMaxSpend:     numOrZero('stackMaxSpend', 0),
        stackCap:          numOrZero('stackCap', 0),
        stackCapStat:      str('stackCapStat', ''),
        stackScaling:      numOr('stackScaling', 1.0),
        stackSpreadBudget: numOrZero('stackSpreadBudget', 0),
