...
Hooks.once('ready', async function () {
  ...
  game.socket.on('system.aspects-of-power', async (payload) => {
    ...
    } else if (['gmApplyBuff', 'gmApplyDebuff', 'gmApplyRestoration', 'gmApplyRepair', 'gmApplyCleanse', 'gmCurseOp', 'gmUpdateDefensePool', 'gmSpendResource', 'gmConsumeReaction', 'gmExecuteTrade', 'gmCreateAoeRegion', 'gmDeleteAoeRegion', 'gmUpdateAoeAffected', 'gmOverworldNote', 'gmEnsureHexResident', 'gmTomeState'].includes(payload.type)) {
    ...
    btn.addEventListener('click', async () => {
      ...
      if (actualHpLoss > 0) {
        ...
        if (attackerForLifesteal?.system?.overhealth) {
          ...
          // tagConfig.lifesteal, carried on the button. ADDS to the actor-wide
          // passive total above rather than replacing it, so a standing
          // Sanguine Tithe and a dedicated drain both pay out. (The legacy
          // actor-wide FLAG keeps its lifestealPct name; only the authored
          // skill field was renamed.)
          const _skillPct = Number(btn.dataset.lifesteal ?? 0);
